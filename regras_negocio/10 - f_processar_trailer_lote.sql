...
DECLARE
	...
	V_sql TEXT;
BEGIN
	-- 1. Loop para processar arquivos com trailer do lote pendente
	FOR VRecord IN
		SELECT DISTINCT 
			ra_h.id_arquivo, 
		...
		WHERE ra_h.numero_linha = 1
		  ...
		  -- O trailer do lote ainda não deve estar processado para este arquivo
		  AND NOT EXISTS (
			  SELECT 1 
			  FROM public.trailer_lote tl 
			  WHERE tl.id_arquivo = ra_h.id_arquivo
		  )
		  -- Mas o layout deve ter campos de tipo_campo = 4 (Trailer Lote)
		  AND EXISTS (
			  SELECT 1 
			  FROM public.leiaute_campo_arquivo lca 
			  WHERE lca.id_leiaute_arquivo = CAST(ra_h.conteudo_jsonb ->> 'id_leiaute_arquivo' AS BIGINT)
			    AND lca.tipo_campo = 4
		  )
	...
	LOOP
		...
		-- 2. Atualiza o conteudo_jsonb de registro_arquivo (linhas > 1) que combinem com as constantes do trailer de lote
		WITH tl_json AS (
			SELECT 
			...
			WHERE ra.id_arquivo = V_id_arquivo
			  ...
			  AND lca.tipo_campo = 4 -- Trailer Lote
			  AND lca.nome_coluna IS NOT NULL
			  -- Garante que a linha atende as constantes do Trailer Lote
			  AND NOT EXISTS (
				  SELECT 1 
				  FROM public.leiaute_campo_arquivo lca2
				  WHERE lca2.id_leiaute_arquivo = VLeiauteID
				    AND lca2.tipo_campo = 4
		...
		-- 3. Monta a query dinâmica e executa a inserção do trailer de lote
		SELECT 
			'id_arquivo, id_empresa, id_leiaute_arquivo, tipo_campo, numero_linha, ' || string_agg(quote_ident(lca.nome_coluna), ', ') AS colunas,
			'ra.id_arquivo, ' || COALESCE(V_id_empresa::TEXT, 'NULL') || ', ' || VLeiauteID || ', 4, ra.numero_linha, ' || string_agg('jpr.' || quote_ident(lca.nome_coluna), ', ') AS select_fields
		...
		IF V_colunas IS NOT NULL THEN
			V_sql := format(
				'INSERT INTO public.trailer_lote (%s) ' ||
				'SELECT %s ' ||
				'FROM public.registro_arquivo ra ' ||
				'INNER JOIN jsonb_populate_record(NULL::public.trailer_lote, ra.conteudo_jsonb) jpr ON TRUE ' ||
				'WHERE ra.id_arquivo = %L ' ||
				'  AND ra.numero_linha > 1 ' ||
				'  AND ra.mensagem_erro IS NULL ' ||
				'  AND ra.conteudo_jsonb ->> ''tipo_registro'' = ''5'' ' || -- Garante que é registro tipo 5
				'  AND NOT EXISTS ( ' ||
				'      SELECT 1 FROM public.trailer_lote tl WHERE tl.id_arquivo = ra.id_arquivo AND tl.numero_linha = ra.numero_linha' ||
				'  )',
				V_colunas,
				V_select,
				V_id_arquivo
