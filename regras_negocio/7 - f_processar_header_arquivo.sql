...
DECLARE
	...
	V_sql TEXT;
BEGIN
	-- 1. Atualiza o conteudo_jsonb de registro_arquivo (linha_arquivo = 1) com os campos do header extraídos dinamicamente sem loops
	WITH header_json AS (
		SELECT 
		...
		INNER JOIN public.leiaute_campo_arquivo lca 
			ON lca.id_leiaute_arquivo = CAST(ra.conteudo_jsonb ->> 'id_leiaute_arquivo' AS BIGINT)
	...
	-- 2. Obtém o ID do único layout a ser processado (sem loops)
	SELECT DISTINCT CAST(ra.conteudo_jsonb ->> 'id_leiaute_arquivo' AS BIGINT) INTO VLeiauteID
	FROM public.registro_arquivo ra
	WHERE ra.numero_linha = 1
	  AND ra.mensagem_erro IS NULL
	  AND ra.conteudo_jsonb ->> 'id_leiaute_arquivo' IS NOT NULL
	...
	-- 3. Se houver um layout identificado, monta a query dinâmica e executa a inserção em lote
	IF VLeiauteID IS NOT NULL THEN
		-- Descobre as colunas configuradas na tabela leiaute_campo_arquivo para o layout atual
		SELECT 
			'id_arquivo, id_leiaute_arquivo, tipo_campo, ' || string_agg(quote_ident(lca.nome_coluna), ', ') AS colunas,
			'ra.id_arquivo, (ra.conteudo_jsonb ->> ''id_leiaute_arquivo'')::BIGINT, 1, ' || string_agg('jpr.' || quote_ident(lca.nome_coluna), ', ') AS select_fields
		...
		IF V_colunas IS NOT NULL THEN
			V_sql := format(
				...
				'WHERE ra.numero_linha = 1 ' ||
				'  AND ra.mensagem_erro IS NULL ' ||
				'  AND CAST(ra.conteudo_jsonb ->> ''id_leiaute_arquivo'' AS BIGINT) = %L ' ||
				'  AND NOT EXISTS ( ' ||
